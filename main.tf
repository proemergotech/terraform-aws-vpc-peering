...
resource "aws_route" "peer_from_to_peer_to" {
  ...
  count = "${var.enabled ? "${length(data.aws_route_tables.peer_from_route_tables.ids)}" : 0}"

  route_table_id            = "${element(data.aws_route_tables.peer_from_route_tables.ids, count.index)}"
  destination_cidr_block    = "${data.aws_vpc.peer_to_vpc.cidr_block}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.peer_from_to_peer_to_vpc.id}"
}

resource "aws_route" "peer_to_to_peer_from" {
  count = "${var.enabled ? "${length(data.aws_route_tables.peer_to_route_tables.ids)}" : 0}"

  route_table_id            = "${element(data.aws_route_tables.peer_to_route_tables.ids, count.index)}"
